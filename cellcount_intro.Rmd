...
The *input_data* function generates a Shiny UI where users enter information regarding directories, numerical and categorical data, 
...
*analyze_images* is a simple UI that contains two interactive buttons. "Run Image Analysis" simply runs the image analysis previously outlined in this document and uses the inputted data from the previous function. Once the completion sound has been played, users can click the "Close window" button to exit the UI. Users can review the processed data in their respective directories.
