...
To get started with *cellcount*, users must make a local clone of the repository using programs like [GitHub Desktop](https://desktop.github.com/) or similar programs. Once cloned, open the R project file and install the package locally by selecting 'build' and 'install package'. You should now have the *cellcount* package installed. Use the following code to bring *cellcount* into the R environment:
...
```
```{r,include=FALSE}
images <- list.files("./test_images/Microcystis_F192_40x/", pattern = "tif", full.name = T)
images_names <- list.files("./test_images/Microcystis_F192_40x/", pattern = "tif", full.name = F)
```
...
savdir <- ("./test_data/")
image_savdir <- ("./test_results/")
images <- list.files("./test_images/NZ_Anabena_60x/", pattern = "tif", full.name = T)
images_names <- list.files("./test_images/NZ_Anabena_60x/", pattern = "tif", full.name = F)
...
savdir <- ("./test_data/")
image_savdir <- ("./test_results/")
images <- list.files("./test_images/Shewanella_100x_DAPI/", pattern = "tif", full.name = T)
images_names <- list.files("./test_images/Shewanella_100x_DAPI/", pattern = "tif", full.name = F)
...
*analyze_images_GUI* is a simple UI that contains two interactive buttons. "Run Image Analysis" simply runs the image analysis previously outlined in this document and uses the inputted data from the previous function. Once the completion sound has been played (using the *beepr* package), users can click the "Close window" button to exit the UI. Users can review the processed data in their respective directories.
