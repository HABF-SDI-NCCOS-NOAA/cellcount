...
when analyzing images. End-users may want to try different objectives in order
to reduce detail as much as possible while able to show distinction between
cells, or use another fluorescent dye (i.e. DAPI) in order to the program to
produce more accurate enumeration results.
...
![A color-label image of *Prorocentrum* sp. where high details were shown from
its fluorescing outer membrane, which caused the program to divide and 
enumerate distinct regions shown here.](C:\Users\Tyler.Harman\Desktop\cellcount_work\misc_\Error Images\3214435.png)

## High background intensities

If high background intensities is observed within images, errors could occur
like the one shown below, where the threshold adjustment built into the
"mapped" function cannot separate the high intensity background from the
majority of cells within the image. End-users should re-take these images and
reduce the background intensity as much as possible. Snap images as rapidly as
you can to avoid fluorescent dyes from increasing in fluorescent output - one
of the contributing factors that we have observed throughout the development
of this package.

![A color-label image of *Shewanella* IRI-160 resulting from a high
background intensity error. This caused the program to produce an 
inconclusive image, due to the lack of being able to separate the high
background intensity from the cells within the image.](C:\Users\Tyler.Harman\Desktop\cellcount_work\misc_\Error Images\542365.png)
