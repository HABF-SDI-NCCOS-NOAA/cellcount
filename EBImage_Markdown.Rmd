...
To begin, we generate a data frame for the proceeding count data to be added 
into using the following code:
```{r}
Cell.Count <- data.frame(Image_File_Name = character(0), Cell_Count = numeric(0))
```

Next, end-users will need to generate a couple of directories; one for saving the unique color label images and another for CSV files. In this example, we are using images of *Microcystis* sp. cells that are contained within the package as test data. Please note that your directory addresses will be different. We recommend using a dedicated directory for ***cellcount*** results:
...
Next, end-users will need to read in the images you are interested in processing using the following code. Once again, your directories will be different and must be updated to the correct file address. However, in this example we will be using
...
In addition, end-users will need to assign the file names to a vector; this provides characterization to each file for easy organization:
...
***tiff*** package. In addition, we'll need to re-orient the image as it's uploaded into the environment. The "*readTIFF*" function uploads images in a portrait format - the "*aperm*" function reorients the uploaded image to a landscape format:
...
cell counts subsequently from the "*image_convert function*". End-users can use the "*display*" function from ***EBImage*** to visually generate the produced image from this function.
...
the "*watershed*" function from the ***EBImage*** package comes first in the outlined function compared to the "*image_convert*" function.
...
analyze image results. This could include adjusting the threshold
...
or the tolerance and 'ext' variables. Observe the initial results as indicated
...
Once end-users have made necessary adjustments, use the following for-loop to
analyze images in bulk from a directory, yet one at a time. The for-loop uses the "*image_convert*" function to produce unique color labels on cells within
...
that we previously made. We can use the following code to view the current results:
...
out of this range and dictates a warning within our data frame, "Cell.Count", which is shown below. This function's primary objective is to alert end-users of potential imaging/processing issues. This alerts the end-user to perhaps re-image a particular sample, or to investigate the error further, if any.
...
disseminated by the microscope camera's manufacturer. End-users must refer to 
their camera documentation to calculate the FOV variable correctly.
